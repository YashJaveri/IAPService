...
import { getCompleteUserStats } from '../utils/handle-stat-details'
...
VerifyRoutes.post('/', ErrorProtectedRoute(async (req: any, res, next) => {    
    ...
    {   
        let totalReqs = (await getCompleteUserStats(req.user, new Date().getMonth(), new Date().getFullYear())).totalCount
